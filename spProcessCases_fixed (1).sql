...
AS 
   ...
   11    7/16/25 Added @CurrentCaseId tracking for better error logging
...
BEGIN
   ...
   -- NEW: Add variable to track current CaseId
   ...
   BEGIN
      ...
      PRINT @sMsg
      -- Log with NULL CaseId since no case is being processed yet
   ...
   ***************************************************************************/
   ...
   -- MODIFIED: Include CurrentCaseId OUTPUT parameter
   EXEC @n = spGenerateMFData '', @FPrefix, @bTestMF, @bStatusUpdate, @bSendMail, @bDebug,
                              @CurrentCaseId = @CurrentCaseId OUTPUT
                              
   PRINT ' ==> ' + LTRIM(STR(@n))
   
   -- NEW: If error occurred and we have a CaseId, include it in debug output
   ...
   ***************************************************************************/
   ...
   ERROR_HANDLER:
      -- MODIFIED: Log with actual CaseId if available
      ...
      IF @bSendMail=1
      BEGIN
         -- Include CaseId in error email if available
