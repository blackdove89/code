...
AS 
   ...
   2     7/17/25 Fixed CaseId tracking and error message truncation
...
BEGIN
   ...
   DECLARE @n                          INT
   DECLARE @sErrorText                 VARCHAR(2000)  -- INCREASED FROM 100 to 2000
   ...
   -- NEW: Variables for tracking context
   DECLARE @CurrentCaseId              INT = NULL
   DECLARE @LastProcessedClaim         VARCHAR(9) = NULL
   DECLARE @ProcessingSummary          VARCHAR(MAX) = NULL
   ...
   ***************************************************************************/
   ...
   -- Check if the new version of spGenerateReissueData exists with OUTPUT parameters
   IF EXISTS (SELECT * FROM sys.parameters p 
              JOIN sys.procedures pr ON p.object_id = pr.object_id 
              WHERE pr.name = 'spGenerateReissueData' 
              AND p.name = '@CurrentCaseId' 
              AND p.is_output = 1)
   BEGIN
      -- Use new version with OUTPUT parameter
      EXEC @n = spGenerateReissueData @FPrefix, @bStatusUpdate, @bSendMail,
                                     @CurrentCaseId = @CurrentCaseId OUTPUT
   END
   ELSE
   BEGIN
      -- Use old version without OUTPUT parameter
      EXEC @n = spGenerateReissueData @FPrefix, @bStatusUpdate, @bSendMail
   END
   ...
   ***************************************************************************/
   ...
   BEGIN
      ...
      SET @sErrorText = @sErrorText + 
                       ' (Process Date: ' + CONVERT(VARCHAR(20), @StartTime, 120) +
                       ', Records processed: ' + LTRIM(STR(ISNULL(@n, 0))) +
                       ', Debug: ' + CASE WHEN @bDebug = 1 THEN 'Yes' ELSE 'No' END
                       
      IF @CurrentCaseId IS NOT NULL
         SET @sErrorText = @sErrorText + ', Last CaseId: ' + CAST(@CurrentCaseId AS VARCHAR(20))
   ...
   ***************************************************************************/
   ...
   ERROR_HANDLER:
      -- Enhanced error logging with CaseId tracking
      INSERT INTO tblErrorLog (CaseId, Process, ErrorMsg) 
      VALUES (@CurrentCaseId, 'spProcessReissueCases', @sErrorText)
      
      PRINT 'spProcessReissueCases --> ' + @sErrorText 

      IF @bSendMail=1
      BEGIN
         -- Include full error details in email
         DECLARE @DetailedErrorMsg VARCHAR(2000)
