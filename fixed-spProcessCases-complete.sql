...
AS 
   ...
   11    7/17/25 Fixed CaseId tracking and error message truncation
...
BEGIN
   ...
   ***************************************************************************/
   ...
   -- Check if the new version of spGenerateMFData exists with OUTPUT parameters
   IF EXISTS (SELECT * FROM sys.parameters p 
              JOIN sys.procedures pr ON p.object_id = pr.object_id 
              WHERE pr.name = 'spGenerateMFData' 
   ...
   ***************************************************************************/
   ...
   BEGIN
      ...
      SET @sErrorText = @sErrorText + 
                       ...
                       ', TestMode: ' + CASE WHEN @bTestMF = 1 THEN 'Yes' ELSE 'No' END +
